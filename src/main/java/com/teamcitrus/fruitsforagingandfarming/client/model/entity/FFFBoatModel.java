...
@OnlyIn(Dist.CLIENT)
public class FFFBoatModel extends EntityModel<FFFBoat> {
    ...
    public void render(FFFBoat entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
    ...
    protected void renderPaddle(FFFBoat boat, int paddle, float scale, float limbSwing) {
        float f = boat.getRowingTime(paddle, limbSwing);
        RendererModel renderermodel = this.paddles[paddle];
        renderermodel.rotateAngleX = (float) MathHelper.clampedLerp(-(float) Math.PI / 3F, -0.2617994F, (MathHelper.sin(-f) + 1.0F) / 2.0F);
        renderermodel.rotateAngleY = (float) MathHelper.clampedLerp(-(float) Math.PI / 4F, (float) Math.PI / 4F, (MathHelper.sin(-f + 1.0F) + 1.0F) / 2.0F);
        if (paddle == 1) {
